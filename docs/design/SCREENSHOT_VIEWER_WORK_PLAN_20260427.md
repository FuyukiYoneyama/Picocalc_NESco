...
ROM menu 上で `S` キーを押すと、`0:/screenshots/` 専用の screenshot viewer mode に入る。
...
- `S`: screenshot viewer mode に入る
- footer 表示は `S VIEW` に固定する
...
- ROM menu の通常 footer に `S VIEW` を追加する。
- `S` / `s` key code を追加する。
- `S` で viewer mode に入り、`ESC` で ROM menu に戻る。
...
- `S` / `s` key code が PicoCalc keyboard から来ているかを debug code または UART で確認する。

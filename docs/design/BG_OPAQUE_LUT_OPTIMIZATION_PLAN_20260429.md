...
- `static const BYTE g_bg_opaque4_lut[16][4]` を追加する
- `renderBgTileFull()` では `opaque_hi` / `opaque_lo` を index にして LUT を参照する
- `dst_opaque[0..3]` へ LUT high 側を byte store する
- `dst_opaque[4..7]` へ LUT low 側を byte store する
- byte store は維持する
...
4. build する
5. banner / `.bss` / UF2 SHA-256 / ELF SHA-256 を HISTORY に記録する
6. commit する
7. 実機確認を 1 回行う
8. 実機結果を HISTORY に記録する
9. 採用 / 不採用を判断する

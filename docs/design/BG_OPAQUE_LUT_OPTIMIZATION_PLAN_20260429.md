...
- `static BYTE g_bg_opaque4_lut[16][4]` を追加する
  ...
  - bit 順序は `g_bg_opaque4_lut[m] = { (m >> 3) & 1, (m >> 2) & 1, (m >> 1) & 1, m & 1 }` に固定する
...
4. build する
5. banner / `.data` / `.bss` / RAM 使用量 / UF2 SHA-256 / ELF SHA-256 を控える
6. 実機確認を 1 回行う
...
- `.data` / `.bss` と RAM 使用量全体を確認する

想定 RAM 増加:

- LUT 本体は RAM 側に `16 * 4 = 64 bytes`
- 初期値付き RAM 配置にした場合、増加は `.bss` ではなく `.data` 側に出る可能性がある

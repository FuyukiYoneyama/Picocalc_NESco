...
## 2026-08-22 Mapper coverage reconciliation

直近の mapper 実装・検証結果を、製品側の現在判定として整理した。ここでは公開文書に不要な実在タイトル名を追加せず、mapper 番号と確認範囲だけを記録する。
...
この表は「完全互換」や「全 variant 対応」の宣言ではない。個別 mapper の詳細な fixture、hash、比較条件は、製品公開文書へ実在タイトル名を持ち込まないため、専用の検証記録側で管理する。

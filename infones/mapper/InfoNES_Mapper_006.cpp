...
static BYTE *Map6_Chr_Ram_Alloc = nullptr;
...
void Map6_Release()
{
  delete[] Map6_Chr_Ram_Alloc;
  Map6_Chr_Ram_Alloc = nullptr;
  Map6_Chr_Ram = nullptr;
}
...
void Map6_Init()
{
  ...
  Map6_Release();
  Map6_Chr_Ram_Alloc = new (std::nothrow) BYTE[0x8000];
  if (!Map6_Chr_Ram_Alloc)
  {
    InfoNES_Error("Mapper 6 startup alloc failed [1/1 chr-ram size=%u]", 0x8000);
    Map6_Release();
    return;
  }
  Map6_Chr_Ram = Map6_Chr_Ram_Alloc;

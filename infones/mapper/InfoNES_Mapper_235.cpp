...
static BYTE *Map235_Dram_Alloc = nullptr;

void Map235_Release()
{
  delete[] Map235_Dram_Alloc;
  Map235_Dram_Alloc = nullptr;
  DRAM = nullptr;
}
...
void Map235_Init()
{
  ...
  Map235_Release();
  Map235_Dram_Alloc = new (std::nothrow) BYTE[DRAM_SIZE];
  if (!Map235_Dram_Alloc)
  {
    InfoNES_Error("Mapper 235 startup alloc failed [1/1 dram size=%u]", DRAM_SIZE);
    Map235_Release();
    return;
  }
  DRAM = Map235_Dram_Alloc;
